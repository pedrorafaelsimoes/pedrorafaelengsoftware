...
def salvar_dados():
    ...
    # serve para momentos em que o usuario não preenche de forma correta os campos de texto
    ...
    if not nome:
    ...
    elif len(tel) < 11 or len(tel) > 11 or not tel.isdigit():
        ...
        # cria o sistema de dados da interface
...
# serve para criar a janela do app
root = tk.Tk()
root.title("Gerenciador de Cadastros")
root.geometry("720x576")

# serve para criar campos de texto
...
for i, text in enumerate(labels):
    tk.Label(frame_inputs, text=text, font=font_grande).grid(row=i, column=0, sticky="w", padx=5, pady=2)
    entry = tk.Entry(frame_inputs, width=30, font=font_grande)
...
# serve para criar um titulo e estruturas da tabela  
...
# ajuste da largura das colunas das informações pessoais
